...
! Working variables
    ...
    REAL :: arg1b
    REAL, DIMENSION(2*n_beams) :: arg10
    REAL, DIMENSION(2*n_beams) :: arg10b
    REAL, DIMENSION(2*n_beams) :: arg2
    REAL, DIMENSION(2*n_beams) :: arg2b
    REAL :: tempb
! EXECUTION
! Initialize the mass value
    mass = 0
...
! Aggregate stresses
    ...
    arg2b(:) = margin_sumb
    arg10b = 0.0
    arg10b(:) = EXP(arg10(:))*arg2b(:)
    marginsb = marginsb - pks*arg10b
    db = 0.0
    tb = 0.0
    kib = 0.0
    DO jj=n_beams,1,-1
      ...
      IF (arg1 .EQ. 0.0) THEN
        arg1b = 0.0
      ELSE
        arg1b = sigma_vm2b/(2.0*SQRT(arg1))
      END IF
      CALL POPREAL4(arg1)
      sigma2b = 2*sigma2*arg1b
      taub = 2*tau*3.0*arg1b
      IF (arg1 .EQ. 0.0) THEN
        arg1b = 0.0
      ELSE
        arg1b = sigma_vm1b/(2.0*SQRT(arg1))
      END IF
      CALL POPREAL4(arg1)
      sigma1b = 2*sigma1*arg1b
      taub = taub + 2*tau*3.0*arg1b
      fib = 0.0
      CALL POPREAL4(tau)
      tempb = 2.0*taub/ai
      fib(1) = fib(1) + tempb
...
!============================================================
  SUBROUTINE TRANSFER_FORCES_D(n_nodes, xfem, xfemd, loads, loadsd, f, &
&   fd)
    IMPLICIT NONE
! Input variables
    INTEGER, INTENT(IN) :: n_nodes
    REAL, INTENT(IN) :: xfem(3, n_nodes), loads(n_nodes-1)
    REAL, INTENT(IN) :: xfemd(3, n_nodes), loadsd(n_nodes-1)
! Output variables
...
! Compute the panel lengths along the span axis (y-dim)
    ld = xfemd(2, 2:n_nodes) - xfemd(2, 1:n_nodes-1)
...
! Compute contributions of each horseshoe vortex to the consistent forces
    DO ii=1,n_nodes-1
      ...
      temp = loads(ii)/12
      fd(2*ii) = fd(2*ii) + l(ii)*loadsd(ii)/12 + temp*ld(ii)
      f(2*ii) = f(2*ii) + temp*l(ii)
      fd(2*ii+1) = fd(2*ii+1) + loadsd(ii)/2
      f(2*ii+1) = f(2*ii+1) + loads(ii)/2
      temp = loads(ii)/12
      fd(2*ii+2) = fd(2*ii+2) - l(ii)*loadsd(ii)/12 - temp*ld(ii)
      f(2*ii+2) = f(2*ii+2) - temp*l(ii)
    END DO
...
&                  loadsd)
    CALL TRANSFER_FORCES_D(n_panels + 1, xllt, xlltd, loads, loadsd, f, &
&                    fd)
...
!                fb margins resfem weight
!   with respect to varying inputs: d alpha0 t gama
!   RW status of diff variables: liftexcess:in-killed d:out alpha0:out
!                t:out ksmargin:in-killed resllt:in-killed fb:in-killed
!                margins:in-killed resfem:in-killed gama:out weight:in-killed
!============================================================
...
&                vinf, rhoair, resllt, sref, cl, cd, l, di, loads)
    CALL TRANSFER_FORCES(n_panels + 1, xllt, loads, f)
...
&             , ksmarginb)
    CALL TRANSFER_FORCES_B(n_panels + 1, xllt, xlltb, loads, loadsb, f, &
&                    fb0)
...
&                  loadsb)
    CALL TRANSFER_DISPS_B(n_panels + 1, xfem, d, db, xllt, xlltb)
