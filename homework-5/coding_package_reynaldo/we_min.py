...
plt.title('Margins, weight optimization', fontsize=20)
plt.show()
...
ax.plot(y, Gama, label='Weight Minimization', color='black', linewidth=2.0)
...
for i in range(-1, len(of)+1):
    if i == -1:
        plt.plot(output0[6], 0.0, marker='d', markersize=20.0,
                 color=[0.0, 0.0, 0.0], markeredgecolor='red')
    elif i == len(of):
        plt.plot(output[6], 0.0, marker='o', markersize=20.0 - 10 * (len(of) - i) / len(of),
...
ax.set_xlim(8890, 9220)
# Customizing the plot:
ax.get_yaxis().set_visible(False)
ax.tick_params(axis="x", labelsize=10)
plt.xlabel('$W0$ value', fontsize=12)
ax.set_facecolor('white')
ax.patch.set_facecolor('white')
plt.title('$W0$ value evolution, from black to blue', fontsize=20)
plt.show()
